...
Build an EURI-powered agent that accepts a complex natural-language objective and turns it into smaller executable tasks. It must expose planning and tool decisions, persist current execution state, recover from individual failures, and return a consolidated final response.
...
| FR-02 | Use EURI to convert the objective into 3–8 tasks. | Stored state contains ordered typed tasks. |
...
- **Testability:** the model is behind an interface and a deterministic fake can replace EURI.
...
4. If below `MAX_REPLANS`, ask EURI for a different unfinished plan using prior results and the error.
...
- Tests pass without an EURI key.
